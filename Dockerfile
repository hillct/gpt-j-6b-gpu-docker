...
RUN apt update && apt-get install -y apt-utils wget \
# &&  wget -q -O- "https://nvidia.github.io/nvidia-docker/gpgkey" | apt-key add - \
# && wget -q -O- "https://nvidia.github.io/nvidia-docker/ubuntu18.04/nvidia-docker.list" -P /etc/apt/sources.list.d \
  &&  apt update \
  && apt install -y python3 python3-pip git zstd curl\
#  && DEBIAN_FRONTEND=noninteractive TZ=Etc/UTC apt-get install -y nvidia-cuda-toolkit
RUN wget "https://mystic.the-eye.eu/public/AI/GPT-J-6B/step_383500.tar.zstd" \
